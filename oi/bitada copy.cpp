...
signed main() {
    ...
    static int tree2_reverse[3000][3000] = {0};
    for (int i = 0; i < m - 1; i++) {
        ini(a);
        ini(b);
        ...
        tree2_reverse[a - 1][b - 1] = tree2[a - 1].size() - 1;
        tree2_reverse[b - 1][a - 1] = tree2[b - 1].size() - 1;
    }
    ...
    vector<vector<vector<int>>> all_permutations(4);
    for (int i = 0; i <= 3; i++) {
        vector<int> bitada_permut(i);
        for (int j = 0; j < bitada_permut.size(); j++) {
            bitada_permut[j] = j;
        }

        do {
            all_permutations[i].pb(bitada_permut);
        ...
        for (int u : at_depth) {
            ...
            for (int v = 0; v < m; v++) {
                ...
                for (int dir = 0; dir < (1 << tree2[v].size()); dir++) {
                    ...
                    int total_sum = 0;
                    for (vector<int> &bitada_permut : all_permutations[u == 0 ? tree1[u].size() : tree1[u].size() - 1]) {
                        int which_node_to_insert = 0;
                        long long combinations_for_a_permutation = 1;
                        for (int other_node = 0; other_node < tree2[v].size(); other_node++) {
                            ...
                            long long combinations_for_all_dirs = sum_dp[children[bitada_permut[which_node_to_insert]]][to_place_node_id][tree2_reverse[to_place_node_id][v]];
